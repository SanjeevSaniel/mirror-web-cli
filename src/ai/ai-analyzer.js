...
export class AIAnalyzer {
  ...
  initializeOpenAI() {
    ...
    if (!apiKey) {
      console.log(chalk.yellow('⚠️ OpenAI API key not found. AI analysis will be disabled.'));
  ...
  /**
   * Analyze website with AI for perfect replica generation
   */
  async analyzeWebsite(url, html, frameworkAnalysis, assets) {
    ...
    try {
      console.log(chalk.blue('🤖 AI analyzing website for perfect replica generation...'));
      
      const analysis = await this.performChainOfThoughtAnalysis(url, html, frameworkAnalysis, assets);
      return analysis;
  ...
  /**
   * Perform chain-of-thought analysis with GPT-4o
   */
  async performChainOfThoughtAnalysis(url, html, frameworkAnalysis, assets) {
    const prompt = this.buildAnalysisPrompt(url, html, frameworkAnalysis, assets);
    ...
    const response = await this.openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert web developer specializing in website mirroring and exact replica generation. Analyze websites and provide actionable insights for creating perfect offline replicas.'
        },
        {
          role: 'user',
          content: prompt
    ...
    return this.parseAIResponse(response.choices[0].message.content, frameworkAnalysis);
  }
  ...
  buildAnalysisPrompt(url, html, frameworkAnalysis, assets) {
    ...
    const htmlSnippet = html.substring(0, 5000); // First 5KB for analysis
    const framework = frameworkAnalysis.primaryFramework?.name || 'Unknown';
    const assetCount = Object.values(assets).reduce((sum, arr) => sum + arr.length, 0);
...
Please provide your analysis in the following JSON format:
{
  ...
  /**
   * Parse AI response and extract actionable insights
   */
  parseAIResponse(response, fallbackAnalysis) {
    try {
      ...
      console.log(chalk.yellow(`⚠️ Failed to parse AI response: ${error.message}`));
      return {
        ...fallbackAnalysis,
        aiInsights: {
...
Provide actionable optimization steps in JSON format:
{
  ...
  "asset_optimizations": ["asset-specific optimizations"]
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a web performance expert. Provide specific, actionable optimization recommendations.'
          },
          {
            role: 'user',
            content: prompt
  ...
  /**
   * Basic optimizations when AI is not available
   */
  ...
  displayAnalysis(analysis) {
    ...
    if (insights.framework) {
      ...
      console.log(chalk.gray(`  Detected: ${insights.framework.detected || 'Unknown'}`));
      console.log(chalk.gray(`  Confidence: ${Math.round((insights.framework.confidence || 0) * 100)}%`));
      console.log(chalk.gray(`  Complexity: ${insights.framework.complexity || 'Unknown'}`));
      console.log('');
    }

    if (insights.replication) {
      console.log(chalk.cyan('Replication Strategy:'));
      console.log(chalk.gray(`  Approach: ${insights.replication.approach || 'Standard mirroring'}`));
      
      if (insights.replication.challenges && insights.replication.challenges.length > 0) {
        console.log(chalk.yellow('  Challenges:'));
        insights.replication.challenges.forEach(challenge => {
          console.log(chalk.gray(`    • ${challenge}`));
        });
      }
      
      if (insights.replication.solutions && insights.replication.solutions.length > 0) {
        console.log(chalk.green('  Solutions:'));
        insights.replication.solutions.forEach(solution => {
